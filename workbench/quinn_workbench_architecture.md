...
## Extension Points for SCHC Integration

The workbench can be extended to include SCHC compression at several points:

### Option 1: Node-Level Hook (Recommended)

Intercept packets in `Node::enqueue_outbound()` or the `process_buffer_for_node` task:

```rust
// In process_buffer_for_node (network/mod.rs:512)
async fn process_buffer_for_node(
    network: Arc<InMemoryNetwork>,
    node: Arc<Node>,
    mut outbound_rx: UnboundedReceiver<InTransitData>,
    schc_context: Option<SchcContext>,  // NEW: SCHC compression context
) {
    while let Some(mut data) = outbound_rx.next().await {
        // NEW: SCHC compression hook
        if let Some(ctx) = &schc_context {
            let compression_result = ctx.compress(&data.transmit.contents);
            // Log compression stats (observer mode - don't modify packet)
            ctx.report_stats(compression_result);
        }
        // Continue with normal forwarding...
    }
}
```

### Option 2: Link-Level Hook

Intercept in `NetworkLink::send()` for per-link compression:

```rust
// In NetworkLink::send (network/link.rs:127)
pub fn send(&mut self, node: &Node, data: InTransitData, extra_delay: Duration) {
    // NEW: SCHC compression hook
    if let Some(ctx) = &self.schc_context {
        let result = ctx.compress(&data.transmit.contents);
        ctx.report_stats(result);
    }
    // Continue with sending...
}
```

### Option 3: UDP Socket Hook

Intercept in `InMemoryUdpSocket::try_send()` for endpoint-level compression:

```rust
// In InMemoryUdpSocket::try_send (quinn_interop.rs:62)
fn try_send(&self, transmit: &Transmit) -> io::Result<()> {
    // NEW: SCHC compression hook (captures QUIC packets at source)
    if let Some(ctx) = &self.schc_context {
        let result = ctx.compress(transmit.contents);
        ctx.report_stats(result);
    }
    // Continue with sending...
}
...
The workbench includes a comprehensive tracing system:

- **SimulationStepTracer**: Records all packet events (sent, received, dropped, duplicated)
- **Verifier**: Post-simulation validation of network invariants
- **Replay Log**: JSON export of all simulation steps

This ensures that any modifications (including SCHC integration) can be verified for correctness.
